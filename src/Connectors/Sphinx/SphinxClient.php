...
namespace Smf\ConnectionPool\Connectors\Sphinx;
...
class SphinxClient
{
	...
	{
		if ( !($handle->send(substr($data, 0, $length))) )
		{
	...
	function _Connect ()
	{
		...
		$fp = new \Swoole\Coroutine\Client(SWOOLE_SOCK_TCP);

		if($this->_timeout <= 0)
		{
			$connectResult = $fp->connect($host, $port, 10);
		}
		else
		{
			$connectResult = $fp->connect($host, $port, $this->_timeout);
		}
	...
	function _GetResponse ( $fp, $client_ver )
	{
		...
		if ( strlen($header)==8 )
		{
			...
			$left = $len;
			while ( $left>0 && !in_array(@$fp->peek(1), ['', false]))
			{
				$chunk = ($fp->recv());
				if ( $chunk )
				{
